...
is the rule both paths now share: the single-clip commit asks it of the row under the pointer,
beside the overlap check it already made, and `canMoveSelectedClips` asks it of every member's
origin row and landing row, so the group's veto has a fourth way to refuse. Silent, like the
other three. The ticket also named the ripple trim, and that half was wrong: `shiftClipsAfter`
