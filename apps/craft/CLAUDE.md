...
number the take is counted as asking for cannot disagree. `useRecordingSave`'s `hasAudio` cannot either, since
ESCSUITE-70: its microphone half is that same pair, resolved once by the controller as
`micAcquired` and handed to the save path with the blob, so a take whose microphone toggle is on
and whose `acquireStreams` came back with `mic: null` builds no mic companion, is counted as
asking for none, and is stored as having no audio. The
