...
export function useUser(): {
  ...
  const firstName =
    ...
    null
  // Memoize so the returned object/user keep a stable reference across renders.
  // Otherwise any consumer with useEffect([user]) re-fires every render (caused
  // the Downloads "Your Licenses" spinner to loop forever).
  return useMemo(
    () => ({
      isLoaded: !loading,
      isSignedIn: !!user,
