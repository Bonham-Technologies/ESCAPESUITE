...
- A release is cut by adding a changeset and merging it to `main`. `changesets/action` (v2) then pushes a `changeset-release/main` branch with the version bump; the Bonham-Technologies org policy currently does not let Actions open pull requests, so open the "Version Packages" PR by hand (`gh pr create --base main --head changeset-release/main --title "chore: version packages"`). That manual step disappears once "Allow GitHub Actions to create and approve pull requests" is enabled at https://github.com/organizations/Bonham-Technologies/settings/actions. Merging it tags each changed package and creates a per-package GitHub Release, with the craft/artist standalone HTML and the headless-artist kit tarball attached to their respective releases.
- Releases are named after the tag (`<pkg>@<version>`) and their body is that version's entry from the package's `CHANGELOG.md`, written by the action itself — so a versioned package must have a changelog entry or the release step fails.
